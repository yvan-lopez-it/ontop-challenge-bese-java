...
6. [Try it with Postman](#6-try-it-with-postman)
   - [Perform transactionEntity](#perform-transactionEntity)
   - [Get all transactionEntities by recipientEntity (ordered and filtered)](#get-all-transactionEntities-by-recipientEntity-ordered-and-filtered)
...
* Populate the `data.sql` file with the SQL statements needed for initializing your database with
  sample data.
* The file has been preloaded with just 1 `recipientEntity` and 8 `transactionEntities`.
...
### Perform transactionEntity
It performs a transactionEntity between the bank accounts and updates the balance.

### Get all transactionEntities by recipientEntity (ordered and filtered)
It retrieves the transactionEntities ordered by descending “creation date” (created_at) in a paginated table and
